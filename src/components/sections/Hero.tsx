...
export function Hero() {
  ...
  return (
    <section
      ...
      className="relative overflow-hidden bg-[radial-gradient(circle_at_top,color-mix(in_srgb,var(--color-accent)_18%,transparent),transparent_34%),linear-gradient(180deg,var(--color-surface-2),var(--color-base)_42%)] px-5 pb-10 pt-8 sm:px-8 lg:pb-14 lg:pt-8"
    >
      <div className="mx-auto grid max-w-7xl gap-8 lg:min-h-[calc(100vh-5.5rem)] lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] lg:items-center lg:gap-14">
        <div className="relative z-10 lg:pr-4">
          ...
          <h1 className="text-display max-w-2xl text-balance text-[clamp(2.85rem,6.2vw,5.45rem)] leading-[0.92]">
            {hero.title}
          </h1>
          <p className="mt-5 max-w-xl text-lg text-[var(--color-muted)] sm:text-[1.15rem]">
            {hero.subtitle}
          </p>
        ...
        <VenueImage
          ...
          className="aspect-[5/4] min-h-[22rem] shadow-[0_30px_80px_rgba(0,0,0,0.28)] lg:aspect-[6/5] lg:min-h-[34rem]"
        />
      </div>
    </section>
  );
